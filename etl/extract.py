...
def requirements(short_term_plan: bool = False) -> DataFrame:
    ...
    data['Дефицит'] = data['Дефицит'].\
        ...
        ~((data['Некомплектная_отгрузка'] == 1) & (data['Полная_отгрузка'] == 1)),
        (data['Количество в заказе'] - data['Перемещено']).map(lambda x: 0 if x < 0 else x)
    )
...
def nomenclature() -> DataFrame:
    ...
    rename_columns = {
        ...
        'Номенклатура.Единица для отчетов': 'Единица отчета',
        'Номенклатура.Коэффициент единицы для отчетов': 'coeff'
    }
    data = data.rename(columns=rename_columns)
    data['coeff'] = modify_col(data['coeff'], instr=1, space=1, comma=1, numeric=1)
